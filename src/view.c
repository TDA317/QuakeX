...
cvar_t	crosshair_color = {"crosshair_color", "0", true}; // Manoel Kasimier - crosshair
...
float V_CalcRoll (vec3_t angles, vec3_t velocity)
{
	...
	if (cl_nobob.value) // Manoel Kasimier - cl_nobob
		value = 0; // Manoel Kasimier - cl_nobob
//	if (cl.inwater)
//		value *= 6;
...
cvar_t	v_centerspeed = {"v_centerspeed","200"}; // Manoel Kasimier - edited
...
// offsets
	...
	// Manoel Kasimier - cl_nobob - begin
	if (cl_nobob.value)
	for (i=0 ; i<3 ; i++)
		r_refdef.vieworg[i] += scr_ofsx.value*forward[i]
			+ scr_ofsy.value*right[i]
			+ (scr_ofsz.value-2)*up[i];
	else
	// Manoel Kasimier - cl_nobob - end
	for (i=0 ; i<3 ; i++)
		r_refdef.vieworg[i] += scr_ofsx.value*forward[i]
			+ scr_ofsy.value*right[i]
			+ scr_ofsz.value*up[i];
...
//		view->origin[i] += up[i]*bob*0.8;
	...
	if (!cl_nobob.value) // Manoel Kasimier - cl_nobob
		view->origin[2] += bob + 2; // Manoel Kasimier
/* // Manoel Kasimier
	view->origin[2] += bob;
...
void V_Init (void)
{
	...
	Cvar_RegisterVariable (&crosshair_color); // Manoel Kasimier - crosshair
