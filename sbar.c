...
void Sbar_ShowScores (void)
{
	if (sb_showscores)
		return;
	sb_showscores = true;
//	sb_updates = 0; // TDA Removed (no longer needed)
}
...
void Sbar_SoloScoreboard (int x, int y, int vertical) // default 0, -48, 0 // Manoel Kasimier Edited
{
...
// Manoel Kasimier - added and edited a lot of things in this function, I won't even bother myself commenting each one... End
}
...
{
	int x; // TDA HAcky
	x = sbar_x.value; // TDA HAcky
	Sbar_SoloScoreboard (x, -49, 1); // TDA HAcky test thing 
//	Sbar_SoloScoreboard (); // TDA HAcky test replaced
	if (cl.gametype == GAME_DEATHMATCH)
...
//			if (flashon > 1)	// TDA Removed (no longer needed)
//				sb_updates = 0;		// force update to remove flash // TDA Removed (no longer needed)
		}
	}
...
// ammo counts
   ...
   for (i=0 ; i<6 ; i++)
      if (cl.items & (1<<(17+i)))
      {
        ...
        if (!(time && time > cl.time - 2 && flashon )) // TDA Edited
// TDA edited above line and removed below lines because sb_updates is no longer needed or used         
...
//         else
 ...
 //        if (time && time > cl.time - 2) // TDA Removed (no longer needed)
 //           sb_updates = 0; // TDA Removed (no longer needed)
      }
...
void Sbar_DrawFace (int x, int y) // default 112, 0 Manoel Kasimier
{
	int		f, anim;
...
//		Sbar_DrawPic (112, 0, sb_face_invuln);
	...
	{
		anim = 1;
//		sb_updates = 0;		// make sure the anim gets drawn over if (time &&	time > cl.time - 2 && flashon )
	}
	else
		anim = 0;
...
//	if (sb_updates >= vid.numpages) // TDA - commented out to force updates every frame
//		return;			// TDA - Thanks LordHavoc for the simple fix

	scr_copyeverything = 1;

//	sb_updates++; // TDA - not needed 
